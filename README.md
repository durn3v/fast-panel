...
| `scripts/vpn-panel xray-restart` | перезапускает **только процесс** Xray в контейнере (подхват `config.json`, панель не трогает) |
| `scripts/vpn-panel reload-xray` | `xray-restart`, затем рестарт **panel** (если нужен и gRPC/панель) |
